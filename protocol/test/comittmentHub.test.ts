...
describe("CommitmentHub", () => {
  ...
  describe("Commitment Creation", () => {
    ...
    it("Creates commitments from registered template contracts", async () => {
      ...
      expect(
        await commitmentHub.commitTemplateRegistry(CommitType.STANDARD)
      ).to.not.equal(ZERO_ADDRESS)
      const tx = commitmentHub.createCommitment(CommitType.STANDARD, encodedString)
    ...
    it("Allows all user commitments to be retrieved by querying events", async () => {
      const txs = await repeat(commitmentHub.createCommitment, [0, encodedString], 5)
      await waitAll(txs)
